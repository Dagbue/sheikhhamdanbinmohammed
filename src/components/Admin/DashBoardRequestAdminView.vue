<template>
  <div class="alpha">
    <div class="body">
      <h2>Royal Membership Card Applications</h2>
      <div class="row trans-mgt">

        <div class="form-group fg--search">
          <button type="submit" @click.prevent="filterUsers"><i class="fa fa-search"></i></button>
          <input  type="text" class="input" placeholder="Search Applications..." v-model="searchQuery" @input="filterUsers"/>
        </div>
    ...
    <div class="section-5">
      ...
      <div class="empty-state" v-if="this.history.length === 0" >
        ...
        <p class="empty-state-text-2">This is where your Royal Membership Card Applications appear</p>
      ...
      <div class="table" v-if="this.history.length >0" >
        <table>
          ...
          <tr>
            ...
            <td data-label="Front">
              <img :src="child.front" style="width: 25%">
            </td>
            <td data-label="Back">
              <img :src="child.back" style="width: 25%">
            </td>
            <td data-label="Passport Number">{{child.passportNumber}}</td>
...
export default {
  name: "DashBoardRequestAdminView",
    ...
    paginatedItems() {
      ...
      const filteredUsers = this.searchQuery.trim() ?
          this.history.filter(user =>
              user.front.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
              user.back.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
              user.passportNumber.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
  ...
  async created() {
    try {
      ...
      const querySnapshot2 = await getDocs(collection(db, "MembershipCard"));
